...
class ContentEngine(object):
    ...
    :param data_set: A pandas dataset containing two fields: description & id
    :return: Nothin!
    """

    def _train(self, data_set):
        tf = TfidfVectorizer(analyzer='word', ngram_range=(
            1, 3), min_df=0, stop_words='english')
        tfidf_matrix = tf.fit_transform(data_set['description'])
        ...
        for idx, row in data_set.iterrows():
            ...
            similar_items = [(cosine_similarities[idx][i],
                              data_set['id'][i]) for i in similar_indices]
